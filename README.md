...
To validate a schema, we provide the convenience function `validate_schema`, which raises an `InvalidSchemaException` if it does not succeed. Note that this function does *not* presently check column names against the table, and is *not* automatically run when an analysis is created.
